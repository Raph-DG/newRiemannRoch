...
lemma krullDimLE_of_coheight
  ...
  rw [Ring.krullDimLE_iff, ringKrullDim_stalk_eq_coheight Z, hZ]
  rfl
...
def ord [IsLocallyNoetherian X]
  ...
  @Ring.ordFrac (X.presheaf.stalk Z) _ _ _ _ (X.functionField) _ (stalkFunctionFieldAlgebra X Z) _
...
lemma ord_of_isUnit (U : X.Opens)
  ...
  have := @ordFrac_of_isUnit (X.presheaf.stalk x) _ _ _ _ (X.functionField) _
      (stalkFunctionFieldAlgebra X x) _ (X.presheaf.germ U x hx' f) this
  rw[← this]
  simp[Scheme.ord]
  congr 1
