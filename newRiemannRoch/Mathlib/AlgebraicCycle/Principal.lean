...
lemma Specialization.strictMono_val :
  ...
  /-
  This exact proof once worked, but unfortunately things have been "fixed" since, and I guess I
  was abusing defeqs or somehting. TODO: modernise this proof.

  Luckily there's no data here so we don't need to fill this in too urgently
  -/
...
lemma QuasiSober.val_lt_genericPoint_of_closure_ne_top (hp : closure p ≠ ⊤) :
  ...
  simp_all only [Set.top_eq_univ, ne_eq, Subtype.forall]
...
lemma div_locally_finite [IsIntegral X] [nt : IsLocallyNoetherian X]
  ...
  (t ∩ Function.support fun Z : X ↦ if h : coheight Z = 1
    ...
    · let XU := (⊤ : Set X) \ U
      have properClosed : XU ≠ ⊤ ∧ IsClosed XU := by
        have := U.2
        unfold XU
        constructor
...
theorem div_neg [IsIntegral X] [h : IsLocallyNoetherian X]
  ...
  suffices div (-1 * f) (sorry) = div f hf by sorry
