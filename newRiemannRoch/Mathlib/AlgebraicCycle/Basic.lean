...
Currently, the contents of this file are less general than the corresponding file in the pull
request #26304. This is just because I didn't want to muck around with having this repo depend
on some edited version of mathlib, and we need to generalize some instance definitions currently
in Mathlib to get more general behaviour for cycles. For now, everything that was stated in terms
of `AddMonoids` in the PR is now stated in terms of `AddCommGroup`s, meaning currently we can't
have cycles with coefficients in `ℕ`.

Once the PR #35807 changing these instances is merged, I'll generalize these definitions in this
repo again.
-/
...
/--
Should be in another file, but is part of the same PR so what are you going to do.
-/
lemma Function.locallyFinsupp.locallyFiniteSupport {X Y : Type*} [TopologicalSpace X] [Zero Y]
    (f : locallyFinsupp X Y) :
...
-/
def Grading.homogeneousCycles {N : Type*} (g : Grading X N) (R : Type*) [AddCommGroup R] (d : N) :
    AddSubgroup (AlgebraicCycle X R) where
...
-/
def dimensionGradingAddSubmonoid [AddCommGroup R] (d : ℕ∞) :
    AddSubgroup (AlgebraicCycle X R) where
...
variable [Ring R] {W : Y.Opens} (hW : IsAffineOpen W)
