...
import newRiemannRoch.Mathlib.AlgebraicCycle.Principal
...
def neg_mema (D : AlgebraicCycle X ℤ) (U : X.Opens) (f : X.functionField) (hf : f ∈ carrier D U) : (- f) ∈ carrier D U := by
  simp_all [carrier]
...
def moduleInstEmpty (D : AlgebraicCycle X ℤ) (hU : ¬ Nonempty U) :
...
lemma something [Nonempty U] {a : Γ(X, U)} {m : carrier D U} :
    ((a • m : carrier D U) : X.functionField) = a • (m : X.functionField) := rfl
  ...
  exact {
    ...
    map_smul' m a := by
      ...
      · rw [ModuleCat.restrictScalars.smul_def
        ...
        have := something (a := (X.sheaf.obj.map r) m) (m := mapFun D (leOfHom (unop r)) a)
        rw [coe_smul] at this
...
lemma presheaf.map_eq' (D : AlgebraicCycle X ℤ) {U V : (TopologicalSpace.Opens ↥X)ᵒᵖ}
    ...
    AddCommGrpCat.ofHom (AddMonoidHom.mk' (map D r) (by intro a b ;simp; sorry)) := rfl
